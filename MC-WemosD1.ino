 ...
 * Sensor :
 *      DHT22 : D9 / 2 /IO2
 *      BH1750: D14 (SDA) / 4 / IO4, D15(SCL) / 5 / IO5, ADDR -> GND
 *      Soil Moisture : A0, VCC -> D10 (SS) / 15
 * 
...
void connectWifi(int toggle){
  if(toggle == 1){
//    Serial.println("Trying smartgarden...");
//    WiFiMulti.addAP("smartgarden", "aduhlupaeuy");
    Serial.println("Trying UnpadWifi...");
...
void loop() {
  ...
  soil = constrain(soil, 585, 1013);
  soil = map(soil, 585, 1013, 100, 0);
  Serial.print(soil);  
  Serial.println("%");  
  digitalWrite(soil_vcc, LOW);
