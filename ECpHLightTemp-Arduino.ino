// Sensor Code Integrated (pH, EC, Temperature, Light) 

// # Connection:
// #        pH meter output     -> Analog pin 0
...
void loop() {
 ...
 for(int i=0;i<10;i++)       //Get 10 sample value from the sensor for smooth the value
  ...
  for(int i=2;i<8;i++)                      //take the average value of 6 center sample
    avgValue+=buf[i];
  float phValue=(float)avgValue*5.0/1024/5; //convert the analog into millivolt
  phValue=3.5*phValue;                      //convert the millivolt into pH value
    ...
    else if(CoefficientVolatge>3300)Serial.print("Out of the range!");  //>20ms/cm,out of the range
    else
    { 
